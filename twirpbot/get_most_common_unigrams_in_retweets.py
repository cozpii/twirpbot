...
def get_most_common_unigrams_in_retweets(name, maxTweets):
    ...
    while count < maxTweets:
        if id_of_earliest_tweet <= 0:
            statuses = twitter.get_user_timeline(screen_name=name, count="100", tweet_mode='extended', include_rts='true', exclude_replies='true')

        else:
            statuses = twitter.get_user_timeline(screen_name=name, count="100", tweet_mode='extended', max_id=id_of_earliest_tweet - 1, include_rts='false', exclude_replies='true')
